...
import { HOST_ALLOW_ENV } from '../auth/api-url';
...
/**
 * Environment the service-launched daemon runs with. launchd, systemd and the
 * Windows startup script do not inherit the installing shell, so anything the
 * daemon needs from the environment has to be written into the unit here.
 *
 * NODE9_API_HOST_ALLOW is the self-hosted escape hatch for the apiUrl pin
 * (auth/api-url). The pin runs inside the daemon too (daemon/sync.ts
 * readCredentials), so without this a self-hosted operator's daemon rejects
 * its own host and ships the device key to api.node9.ai on every tick while
 * the hooks, which inherit the shell, keep working. Captured at
 * `node9 daemon install`; changing it means reinstalling, the same lifecycle
 * as everything else in the unit.
 */
export function daemonServiceEnv(): Record<string, string> {
  const env: Record<string, string> = { NODE9_AUTO_STARTED: '1' };
  const allow = process.env[HOST_ALLOW_ENV]?.trim();
  // A suffix list is letters, digits, dots, commas, wildcards and spaces.
  // Anything else is not a hostname and must not reach a unit file, a plist
  // or a VBS string literal.
  if (allow && /^[A-Za-z0-9.*,_ -]+$/.test(allow)) env[HOST_ALLOW_ENV] = allow;
  return env;
}

export function launchdPlist(binaryPath: string): string {
  ...
  const envEntries = Object.entries(daemonServiceEnv())
...
export function systemdUnit(binaryPath: string): string {
  ...
  const envLines = Object.entries(daemonServiceEnv())
